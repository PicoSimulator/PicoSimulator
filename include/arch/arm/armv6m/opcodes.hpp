...
{\
  ...
  switch(sub_opcode) {\
    ...
    case 0b0111:{\
      uint32_t Rdn = (opcode >> 16) & 0x07;\
      uint32_t Rm = (opcode >> 19) & 0x07;\
      do_disasm({std::cout << FMT_op16(opcode) << FMT_dis_RdnRm("ROR", Rdn, Rm) << std::endl; })\
      do_exec({\
        uint32_t Rdn_val = get_reg(Rdn);\
        uint32_t Rm_val = get_reg(Rm)%32;\
        Rdn_val = (Rdn_val >> Rm_val) | (Rdn_val << (32-Rm_val)); \
        bool carry = Rdn_val&(1<<31);\
        set_reg(Rdn, Rdn_val); \
        SETFLAGS_NZC(Rdn_val, carry);\
        do_trace({\
          std::cout << FMT_reg(Rdn) << ":=0x" << FMT_hex(Rdn_val)  \
