...
namespace ARMv6M{
  ...
  private:
    ...
    class SysTick final : public IClockable{
    public:
      SysTick(const std::string &name) : m_vcd{name} {
        m_vcd.add_item(m_CSR);
        m_vcd.add_item(m_RVR);
        m_vcd.add_item(m_CVR);
        m_vcd.add_item(m_CALIB);
      }
      virtual void tick() override
      {
        m_CVR++;
      }
      uint32_t CSR() const { return m_CSR; }
      uint32_t RVR() const { return m_RVR; }
      uint32_t CVR() const { return m_CVR; }
      uint32_t CALIB() const { return m_CALIB; }
      Tracing::VCD::Register<uint32_t> &CSR() { return m_CSR; }
      Tracing::VCD::Register<uint32_t> &RVR() { return m_RVR; }
      Tracing::VCD::Register<uint32_t> &CVR() { return m_CVR; }
      Tracing::VCD::Register<uint32_t> &CALIB() { return m_CALIB; }

      Tracing::VCD::Module &vcd() { return m_vcd; }
    protected:
    private:
      Tracing::VCD::Register<uint32_t> m_CSR{"CSR"};
      Tracing::VCD::Register<uint32_t> m_RVR{"RVR"};
      Tracing::VCD::Register<uint32_t> m_CVR{"CVR"};
      Tracing::VCD::Register<uint32_t> m_CALIB{"CALIB"};
      Tracing::VCD::Module m_vcd;
    };
    
    SysTick &systick() { return m_systick; }
