...
public:
  ...
  void schedule_in(simulation_time_t time, IClockable &c) {
    m_schedule.push({m_time + time, c, 0});
  }
  void schedule_periodic(simulation_time_t period, IClockable &c)
  {
    m_schedule.push({m_time + period, c, period});
  }
  constexpr static unsigned to_seconds(simulation_time_t t) { return t / 10'000'000'000; }
  ...
  constexpr static simulation_time_t from_nanoseconds(unsigned ns) { return ns*10; }
...
private:
  ...
  struct Event{
    ...
    std::reference_wrapper<IClockable> clockable;
