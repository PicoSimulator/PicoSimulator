...
{
  std::cout << "MPU::read_halfword(" << std::hex << addr << std::dec << ")" << std::endl;
  co_return co_await m_bus_interface.read_halfword(addr);
}

Awaitable<uint32_t> ARMv6MCore::MPU::read_word(uint32_t addr)
{
  std::cout << "MPU::read_word(" << std::hex << addr << std::dec << ")" << std::endl;
  if ((addr & 0xe000'0000) == 0xe000'0000) {
    uint32_t out;
    m_core.m_ppb.read_word(addr, out);
    co_return out;
  } else {
    co_return co_await m_bus_interface.read_word(addr);
  }
}
...
{
  std::cout << "MPU::write_word(" << std::hex << addr << ", " << data << std::dec << ")" << std::endl;
  if ((addr & 0xe000'0000) == 0xe000'0000) {
